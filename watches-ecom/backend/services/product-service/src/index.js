...
app.post("/api/products", async (req, res) => {
  const { name, brand, category, priceCents } = req.body || {};
  ...
  try {
    ...
    try {
      await client.query(`SET search_path TO ${schema}`);
      await client.query(
        "INSERT INTO watches (id, name, brand, category, price_cents) VALUES ($1, $2, $3, $4, $5)",
        [id, name, brand, category, priceCents]
      );
    } finally {
      client.release();
    }

    return res.status(201).json({ id, name, brand, category, priceCents });
  } catch (error) {
